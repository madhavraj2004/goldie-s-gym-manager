...
serve(async (req) => {
  ...
  try {
    ...
    const { email, password, full_name, phone, specialization, certifications, experience_years, bio } = body;

    if (!email || !password || !full_name) {
      throw new Error("email, password, and full_name are required");
    }

    // Create auth user with trainer role
    ...
    // Update trainer profile (trigger already creates it)
    ...
    if (Object.keys(updates).length > 0) {
      await supabaseAdmin.from("trainer_profiles").update(updates).eq("user_id", userId);
    }
